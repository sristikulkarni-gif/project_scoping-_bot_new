...
async def _fetch_related_case_study(project_id: uuid.UUID, db: AsyncSession) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        # Search Qdrant
        # Note: qdrant-client v1.10+ uses query_points instead of search
...
# PPTX PREVIEW
@router.post("/preview/pptx")
async def preview_pptx_from_scope(
    project_id: uuid.UUID,
    scope: Optional[Dict[str, Any]] = None,
    db: AsyncSession = Depends(get_async_session),
    current_user: models.User = Depends(current_active_user),
    ...
    try:
        file = await export.generate_pptx(normalized)
        safe_name = _safe_filename(normalized.get("overview", {}).get("Project Name") or f"project_{project_id}")
        # Use standard Response for binary file to avoid streaming generator issues with BytesIO
        from fastapi import Response
        return Response(
            content=file.getvalue(),
            media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation",
            headers={"Content-Disposition": f"attachment; filename={safe_name}_{project_id}_preview.pptx"},
        )
    except Exception as e:
...
@router.get("/smart-pptx")
async def export_project_smart_pptx(
    project_id: uuid.UUID,
    db: AsyncSession = Depends(get_async_session),
    current_user: models.User = Depends(current_active_user),
):
    """
    Generates a 'Smart' AI-written Pitch Deck for the project.
    """
    from app.utils.presentation_generator import generate_smart_pptx, TEMPLATE_PROMPT
    ...
    try:
        # Generate the PPTX
        file = await generate_smart_pptx(scope)
        safe_name = _safe_filename(project.name or f"project_{project.id}")
        
        # Use standard Response for binary file
        from fastapi import Response
        return Response(
            content=file.getvalue(),
            media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation",
            headers={"Content-Disposition": f"attachment; filename={safe_name}_{project.id}_PITCH_DECK.pptx"},
        )
    except Exception as e:
        logger.error(f"Smart PPTX generation failed: {e}")
        import traceback
        traceback.print_exc()
        raise HTTPException(status_code=500, detail=f"Smart PPTX generation failed: {str(e)}")
...
async def export_project_pptx(
    ...
    try:
        file = await export.generate_pptx(scope)
        safe_name = _safe_filename(project.name or f"project_{project.id}")
        # Use standard Response for binary file to avoid streaming generator issues with BytesIO
        from fastapi import Response
        return Response(
            content=file.getvalue(),
            media_type="application/vnd.openxmlformats-officedocument.presentationml.presentation",
            headers={"Content-Disposition": f"attachment; filename={safe_name}_{project.id}.pptx"},
        )
    except Exception as e:
        logger.error(f"PPTX generation failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))