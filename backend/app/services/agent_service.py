...
class ProjectScopingAgent:
    ...
    def __init__(self):
        ...
        try:
            self.llm = AzureChatOpenAI(
                ...
                request_timeout=120,  # 2 min timeout for large structured outputs
                max_tokens=16384,     # Large enough for full scope generation
            )
    ...
    async def generate_scope(
        ...
        try:
            ...
            # --- PHASE 0: LLM-BASED TECH EXTRACTION START ---
            logger.info("🤖 Agent starting Phase 0: Technology Extraction from RFP...")
            try:
                from app.schemas import TechExtractionOutput
                tech_extract_prompt = (
                    "Extract ALL specific technologies, tools, platforms, frameworks, "
                    "programming languages, databases, cloud services, and software products "
                    "that are EXPLICITLY mentioned in the following RFP document. "
                    "Only include what is directly stated in the text, do NOT infer or suggest "
                    "technologies that are not mentioned.\n\n"
                    f"RFP Document:\n{rfp_text[:5000]}"
                )
                tech_extract_llm = self.llm.with_structured_output(TechExtractionOutput)
                tech_result = await tech_extract_llm.ainvoke([
                    HumanMessage(content=tech_extract_prompt)
                ])
                extracted_explicit_stack = tech_result.technologies
                explicit_stack_str = ", ".join(extracted_explicit_stack) if extracted_explicit_stack else "None strictly identified."
                logger.info(f"🔍 Phase 0 Extracted Explicit Stack: {explicit_stack_str}")
            except Exception as e:
                logger.warning(f"⚠️ Phase 0 tech extraction failed, continuing without: {e}")
                explicit_stack_str = "None strictly identified."
            # --- PHASE 0: LLM-BASED TECH EXTRACTION END ---
...
1. Extract the optimal high-level phases (e.g., Discovery, Design, Build, QA) and required team roles. The required team roles MUST EXACTLY MATCH a role from the provided Available Rate Cards list below. You are STRICTLY FORBIDDEN from inventing or assigning any roles (e.g. "Change Management Lead", "Scrum Master") that do not exist in the Available Rate Cards json.
...
2. Every activity MUST be owned by one of the Team Roles listed above. The assigned role MUST EXACTLY MATCH a role from the Available Rate Cards list below. You are STRICTLY FORBIDDEN from inventing or assigning any roles (e.g. "Change Management Lead", "Scrum Master") that do not exist in the provided Available Rate Cards json.
