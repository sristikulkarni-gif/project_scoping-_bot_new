...
async def init_container():
    try:
        ...
        print(f"Warning: Failed to create Azure Blob container: {e}")
...
async def upload_bytes(
    ...
    for attempt in range(3):
        try:
            # Added large timeouts to prevent "Timeout on reading data from socket" on larger files
...
async def download_bytes(blob_name: str, base: str = "", timeout: int = 300) -> bytes:
    ...
    # Check existence first so it fails fast instead of hanging/retrying for 3 mins
    try:
        if not await blob.exists():
            raise ResourceNotFoundError(f"Blob {path} does not exist")
    except Exception as e:
        raise ResourceNotFoundError(f"Cannot access blob {path}: {e}")
