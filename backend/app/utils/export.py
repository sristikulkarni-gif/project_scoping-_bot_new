...
async def generate_pptx(scope: Dict[str, Any]) -> io.BytesIO:
    """Generate a PowerPoint presentation from project scope."""
    data = scope or {}
    prs = Presentation()
    ...
    project_name = data.get("overview", {}).get("Project Name", "Project Scope")
    title.text = project_name
    subtitle.text = f"Project Scoping Report\nGenerated on {datetime.now().strftime('%Y-%m-%d')}"
    ...
    if exec_summary:
        ...
        tf = body.text_frame
        tf.text = exec_summary[:1000] # Truncate if too long
    ...
    if arch_path:
        try:
            ...
            # PPTX only supports raster images (JPG, PNG). SVG is not supported directly.
            img_bytes = None
            try:
                img_bytes = await azure_blob.download_bytes(arch_path, timeout=15)
            except Exception:
                # If specific blob not found, try to fallback (e.g. if .svg check for .png or vice versa)
                # But here, if we have .svg, we can't use it anyway.
                # Just catch the error to proceed.
                pass
            
            # If failed or empty (or if it was an SVG that we successfully downloaded but can't use)
            is_svg = arch_path.lower().endswith(".svg")
            
            if not img_bytes and not is_svg:
                # Try fallback: if .png missing, check if .svg exists just to report correct status
                ...
                try:
                    ...
                    if not img_bytes:
                        if arch_path.endswith(".png"):
                           ...
                           svg_path = arch_path
                        img_bytes = await azure_blob.download_bytes(svg_path, timeout=15)
                    ...
                    if img_bytes:
                        ...
                        try:
                            ...
                            script_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "..", "convert_svg.js")
                            if os.path.exists(script_path):
                                ...
                                if os.path.exists(temp_png):
                                    ...
                                    is_svg = False # Treated as PNG now
            ...
            if is_svg:
                # Still SVG? Meaning conversion failed or skipped. Use placeholder.
                # Handle SVG case: Add a placeholder text instead of crashing or showing nothing
                slide_layout = prs.slide_layouts[1] # Title/Content
                slide = prs.slides.add_slide(slide_layout)
                title = slide.shapes.title
                title.text = "System Architecture"
                tf = slide.shapes.placeholders[1].text_frame
                tf.text = "[Architecture Diagram is in SVG format and could not be converted for PowerPoint]\n\nPlease view the high-resolution diagram in the web application."
            ...
            logger.warning(f"Failed to add architecture diagram to PPTX: {e}")
    ...
    if activities:
        ...
        slide = prs.slides.add_slide(slide_layout)
        title = slide.shapes.title
        title.text = "High-Level Timeline"
        
        # Simple list of phases/activities
        # For a true Gantt in PPTX, we'd need to draw shapes, which is complex.
        # We'll list key activities and dates.
        tf = slide.shapes.placeholders[1].text_frame
        tf.text = "Key Phases & Milestones"
        
        for act in activities[:8]: # Limit to top 8 to fit
            p = tf.add_paragraph()
            p.text = f"• {act.get('Activities', 'Task')} ({act.get('Start Date', '')} - {act.get('End Date', '')})"
            p.level = 1
    ...
    if plan:
        ...
        slide = prs.slides.add_slide(slide_layout)
        title = slide.shapes.title
        title.text = "Resourcing & Cost Estimates"
        
        # Create Table
        rows = len(plan) + 2 # Header + Total
        cols = 4 # Role, Rate, Efforts, Cost
        ...
        table.cell(0, 1).text = "Rate/Mo"
        table.cell(0, 2).text = "Efforts (Mo)"
        ...
        for i, row in enumerate(plan):
            ...
            table.cell(r_idx, 0).text = str(row.get("Resources", ""))
            table.cell(r_idx, 1).text = f"{float(row.get('Rate/month', 0)):,.0f}"
            table.cell(r_idx, 2).text = f"{float(row.get('Efforts', 0)):.1f}"
            table.cell(r_idx, 3).text = f"{cost:,.0f}"
    ...
    # --- Related Case Study ---
    case_study = data.get("related_case_study", {})
    if case_study and case_study.get("matched"):
        cs = case_study.get("case_study", {})
        slide_layout = prs.slide_layouts[1]
        slide = prs.slides.add_slide(slide_layout)
        title = slide.shapes.title
        title.text = f"Reference: {cs.get('client_name', 'Case Study')}"
        
        tf = slide.shapes.placeholders[1].text_frame
        tf.text = "Similar Project Reference"
        
        p = tf.add_paragraph()
        p.text = f"Overview: {cs.get('overview', '')[:200]}..."
        p.level = 1
        
        p = tf.add_paragraph()
        p.text = f"Impact: {cs.get('impact', '')[:200]}..."
        p.level = 1
        
        p = tf.add_paragraph()
        p.text = f"Similarity Match: {case_study.get('similarity_score', 0)*100:.1f}%"
        p.level = 1
